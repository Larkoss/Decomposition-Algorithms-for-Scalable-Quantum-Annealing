...
for i in range(5): #10 - 125
    for j in range(9): #0.1 - 0.9
        for x in range(3): #0.1 - 0.9
            ...
            start = time.time()
            solution_original = DBR(G, 10 + x * 5, minimum_vertex_cover_exact_solve_np_hard)
            end = time.time()
            ...
            start = time.time()
            solution_pegasus = DBR(G2, 10 + x * 5, minimum_vertex_cover_pegasus)
            end = time.time()
            ...
            start = time.time()
            solution_chimera = DBR(G3, 10 + x * 5, minimum_vertex_cover_chimera)
            end = time.time()
